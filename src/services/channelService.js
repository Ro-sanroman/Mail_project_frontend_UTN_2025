...
async function inviteToChannel(workspace_id, channel_id, email, role) {
  const url =
    ENVIRONMENT.URL_API +
    `/api/workspace/${workspace_id}/channels/${channel_id}/members/invite`;
  ...
  console.log(
    "[channelService][INVITE][POST] status:",
    response_http.status,
    "response:",
    response
  );
...
async function inviteToWorkspace(workspace_id, email, role) {
  ...
  console.log(
    "[channelService][INVITE_WS][POST] status:",
    response_http.status,
    "response:",
    response
  );
  if (!response_http.ok || response?.ok === false) {
    throw new Error(
      response?.message || "Error al invitar usuario al workspace"
    );
  }
  return response;
}
